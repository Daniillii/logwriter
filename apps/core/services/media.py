...
class MediaService:
    ...
    def is_allowed_extension(file: UploadFile):
        if file.content_type not in ["image/jpeg", "image/png", "image/gif"]:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Некорректный формат файл")
        return True

    @staticmethod
    async def is_allowed_file_size(file: UploadFile):
        file_size_in_bytes = MAX_FILE_SIZE * 1024 * 1024 

        file_size = len(await file.read())
        if file_size > file_size_in_bytes:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail=f"Файл превышает размер {file_size_in_bytes}MB")
