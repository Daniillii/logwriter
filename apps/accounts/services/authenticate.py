...
class AccountService:
    ...
    def register(cls, email: str, password: str):
        ...
        if UserManager.get_user(email=email):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Этот Email уже занят."
            )

        new_user = UserManager.create_user(email=email, password=password)
        token = TokenService(new_user.id)
        token.request_is_register()
        EmailService.register_send_verification_email(new_user.email)

        return {
            "email": new_user.email,
            "message": "Пожалуйста, проверьте свою электронную почту на наличие КОДА для подтверждения адреса электронной почты.",
        }
    ...
    def verify_registration(cls, email: str, otp: str):
        ...
        if user.is_verified_email:
            raise HTTPException(
                ...
                detail="Этот адрес электронной почты уже проверен.",
            )
        ...
        if not token.validate_otp_token(otp):
            raise HTTPException(
                ...
                detail="Неверный код. Пожалуйста, перепроверьте и попробуйте еще раз.",
            )
        ...
        return {
            ...
            "message": "Ваш адрес электронной почты подтвержден. Учетная запись успешно активирована.",
        }
    ...
    def login(cls, email: str, password: str):
        ...
        if not user.is_active:
            raise HTTPException(
                ...
                detail="Неактивная учетная запись.",
                headers={"WWW-Authenticate": "Bearer"},
            )
    ...
    def reset_password(cls, email: str):
        ...
        return {
            "message": "Пожалуйста, проверьте свою электронную почту на наличие кода для подтверждения запроса на сброс пароля."
        }
    ...
    def verify_reset_password(cls, email: str, password: str, otp: str):
        ...
        return {"message": "Ваш пароль был изменен."}
    ...
    def change_email(cls, user, new_email):
        ...
        if UserManager.get_user(email=new_email) is None:
            ...
            raise HTTPException(
                ...
                detail="Это письмо уже было получено.",
            )

        return {
            "message": f'Пожалуйста, проверьте свою электронную почту "{new_email}" для получения кода для подтверждения запроса на изменение.'
        }
    ...
    def verify_change_email(cls, user, otp):
        ...
        if token.validate_otp_token(otp):
            ...
            if new_email:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Неверный запрос на проверку электронной почты.",
                )
        else:
            raise HTTPException(
                status_code=status.HTTP_406_NOT_ACCEPTABLE,
        ...
        return {"message": "Ваш адрес электронной почты изменен."}
    ...
    def resend_otp(cls, request_type: str, email: str):
        ...
        if current_request_type != request_type:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Текущий запрошенный тип недействителен.",
            )
