...
class TokenService:
    ...
    def create_access_token(self) -> str:
        ...
        to_encode.update(
            {
                "exp": datetime.now(tz=pytz.timezone("Europe/Moscow"))
    ...
    def validate_otp_token(cls, token: str):
        ...
        return totp.verify(token, for_time=datetime.now(pytz.timezone("Europe/Moscow")))
    ...
    def check_time_remaining(cls):
        ...
        time_remaining = int(totp.interval - datetime.now(tz=pytz.timezone("Europe/Moscow")).timestamp() % totp.interval)
        if time_remaining != 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Срок действия Кода не истек. Повторная отправка доступна через {time_remaining} секунд.",
            )
